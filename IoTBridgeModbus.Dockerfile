...
ARG MODBUS_HOST
ENV MODBUS_HOST=""
LABEL env.MODBUS_HOST="Modbus gateway to connect to."

ARG MODBUS_TLS
ENV MODBUS_TLS="false"
LABEL env.MODBUS_TLS="If Modbus gateway connection should use TLS."

ARG MODBUS_PORT
ENV MODBUS_PORT="502"
LABEL env.MODBUS_PORT="Optional Port number to use when connecting to host."
...
COPY [ \
	...
	"ConsoleBridge/bin/Release/PublishOutputLinux/linux-x64/Waher.Networking.Modbus.dll", \
	...
	"ConsoleBridge/bin/Release/PublishOutputLinux/linux-x64/Waher.Things.Modbus.dll", \
