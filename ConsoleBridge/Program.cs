...
internal class Program
{
	...
	private static async Task Main()
	{
		try
		{
			...
			Types.Initialize(
				...
				typeof(ModbusGatewayNode).GetTypeInfo().Assembly,
			...
			bool ModbusGatewayFound = false;

			foreach (INode Node in await MeteringTopology.Root.ChildNodes)
			{
				if (Node is ModbusGatewayNode)
				{
					ModbusGatewayFound = true;
					break;
				}
			}
			...
			if (!ModbusGatewayFound)
			{
				...
				while (true)
				{
					if (!string.IsNullOrEmpty(ModbusHost))
					{
						try
						{
							SafeDispose(ref ModbusClient);

							Log.Informational("Connecting to " + ModbusHost + ":" + ModbusPort.ToString());

							ModbusClient = await ModbusTcpClient.Connect(ModbusHost, ModbusPort, ModbusTls);
							SafeDispose(ref ModbusClient);

							if (Updated)
							{
								await RuntimeSettings.SetAsync("ModbusHost", ModbusHost);
								await RuntimeSettings.SetAsync("ModbusPort", ModbusPort);
								await RuntimeSettings.SetAsync("ModbusTls", ModbusTls);
							}
					...
					ModbusTls = InputString("Modbus Encryption", ModbusTls);
					ModbusPort = InputString("TCP/IP Port", ModbusPort);
					Updated = true;
				}

				ModbusGatewayNode ModbusNode = new()
				{
					NodeId = await MeteringNode.GetUniqueNodeId(ModbusHost),
					Host = ModbusHost,
					Port = ModbusPort,
					Tls = ModbusTls
				};

				await MeteringTopology.Root.AddAsync(ModbusNode);
			}
	...
	private static string GetVCardXml()
	{
		...
		Xml.Append("<FN>Modbus-XMPP Bridge</FN><N><FAMILY>Bridge</FAMILY><GIVEN>Modbus-XMPP</GIVEN><MIDDLE/></N>");
		Xml.Append("<URL>https://github.com/Neuro-Foundation/IoTBridgeModbus</URL>");
		Xml.Append("<JABBERID>");
		...
		Xml.Append(deviceId);
		Xml.Append("</UID>");
		Xml.Append("<DESC>Modbus-XMPP Bridge Project (IoTBridgeModbus), based on a concentrator example from the book Mastering Internet of Things, by Peter Waher.</DESC>");
				...
				try
				{
					List<MetaDataTag> MetaInfo =
						[
							...
							new MetaDataStringTag("TYPE", "Modbus<->XMPP"),
							new MetaDataStringTag("MAN", "neuro-foundation.io"),
							new MetaDataStringTag("MODEL", "IoTBridgeModbus"),
							new MetaDataStringTag("PURL", "https://github.com/Neuro-Foundation/IoTBridgeModbus"),
